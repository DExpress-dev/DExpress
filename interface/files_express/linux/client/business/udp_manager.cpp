...
int udp_manager::connect()
{
	...
	if (ERROR_HANDLE == remote_handle_)
	{
		if(parent_ != nullptr)
		{
...
int udp_manager::send_config(const int64_t &size, const std::string &local_path, const std::string &remote_path, std::string &file_name)
{
	if (ERROR_HANDLE == linker_handle_)
	{
...
int udp_manager::send_file_block(int64_t max, int64_t cur, char* data, int size)
{
	if(ERROR_HANDLE == linker_handle_)
	{
		return -1;
