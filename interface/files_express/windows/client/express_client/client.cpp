...
#endif
{
	...
	if (remote_server_ptr != nullptr)
	{
		...
		else
		{
			...
			while (1)
			{
				...
				//状态判断
				if (nullptr != remote_server_ptr->server_ptr_->udp_manager_ && -1 != remote_server_ptr->server_ptr_->udp_manager_->linker_handle_ && remote_server_ptr->server_ptr_->current_client_state_ == ccs_logined)
				{
	...
	while (1)
	{
		...
		//状态判断
		if (nullptr != express_ptr->udp_manager_ && -1 != express_ptr->udp_manager_->linker_handle_ && express_ptr->current_client_state_ == ccs_logined)
		{
			main_thread::get_instance()->add_log(LOG_TYPE_INFO, "open_client Connect Success!");
		...
		if (flag_count >= connect_count)
		{
			main_thread::get_instance()->add_log(LOG_TYPE_ERROR, "open_client Create New Client Failed Connect Timeout");
